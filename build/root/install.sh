...
packages="kmod openvpn privoxy bind-tools netcat-openbsd ipcalc wireguard-tools openresolv curl"

# install pre-reqs
apk add $packages


### Need to install modules for alpine
...
if [[ "${VPN_ENABLED}" == "yes" ]]; then
	...
	if [[ "${VPN_CLIENT}" == "wireguard" ]]; then
		...
		if [[ -z "${vpn_config_path}" ]]; then
			...
			## Not sure what this does but i think its borking file - clayton
